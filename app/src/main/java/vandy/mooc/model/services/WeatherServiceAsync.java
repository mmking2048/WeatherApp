        ...
        new WeatherRequest.Stub() {
            ...
            public void getCurrentWeather(final String location,
                ...
                Runnable getWeatherRunnable = new Runnable() {
                    @Override
                    public void run() {
                        final List<WeatherData> weatherDatas = getWeatherResults(location);
                        ...
                        try {
                            if (weatherDatas != null) {
                                ...
                                // For testing purposes, this message was originally "You suck!"
                                // A small lesson in psychology: having "You suck!" thrown at you every
                                // time something goes wrong is a rather disheartening experience.
                                callback.sendError("No weather data for location \""
                                        + location
                                        + "\" found");
                            }
                ...
                mExecutorService.execute(getWeatherRunnable);
            }
        };
}
